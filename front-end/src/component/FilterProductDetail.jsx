...
const  FilterProductDetail = ({filteredProducts}) => {
 
  const [qty, setQty] = useState(0);
  const {addCart,addWishList,product,ProductIndex} = useContext(ThemeContext)
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-4">
      ...
      <section className="part2 w-full md:w-2/3 space-y-2">
        <p className="text-gray-600">{filteredProducts ?filteredProducts.productDescription : "Loading..." }</p>
        <h1 className="text-2xl font-semibold">{ filteredProducts? filteredProducts.productName : "Loading..." }</h1>
      <div className="flex items-center gap-4 mt-2">
  {/* Price */}
  <p className="text-[#ff5252] text-xl font-semibold">
    ₹{filteredProducts ? filteredProducts.productPrice : "Loading..."}
  </p>

  {/* Stock Status */}
  {filteredProducts ? (
    filteredProducts.productStock > 0 ? (
      <span className="text-green-500 font-medium">In Stock  ({filteredProducts.productStock})</span>
    ) : (
...
<Rating name="size-small" defaultValue={filteredProducts ? filteredProducts.productDescription : "Loading..." } size="small" readOnly />
...
<input
  ...
  onChange={(e) => {
    ...
    setQty(value < 0 ? 0 : value);
            ...
            onClick={()=>addCart(filteredProducts)}
            >
           ...
           <span className='flex  items-center gap-2 mt-5 text-[15px] link cursor-pointer font-[500]'
            onClick={()=>addWishList(filteredProducts)}
           >
